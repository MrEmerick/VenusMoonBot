const {
    ...
    ButtonStyle, Events, StringSelectMenuBuilder, ChannelType, PermissionsBitField
} = require('discord.js');
...
||@everyone @here||`);
                ...
                await interaction.reply({
                    content: 'Erro ao criar canal. Verifique se o ID da categoria está correto e se o bot tem permissões.',
                    ephemeral: true,
                });
            }
        }
    ...
    if (interaction.isModalSubmit() && interaction.customId === 'set_user_info') {
        ...
        await interaction.reply({
            content: 'Sua solicitação foi enviada para análise. Aguarde a aprovação de um administrador.',
            ephemeral: true,
        });
